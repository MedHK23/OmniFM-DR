...
class AdjustLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def get_lprobs_and_target(self, model, net_output, sample):
        ...
        if "constraint_masks" in sample and sample["constraint_masks"] is not None:
            ...
            net_output[0].masked_fill(~constraint_masks, -math.inf)
    ...
    def _middle_process(self, loss_list, logging_list, sample_size):
        loss = loss_list[0].data
        loss_value = loss_list[0].data
        nll_loss = logging_list[0]['nll_loss'].data
        ntokens = logging_list[0]['ntokens']
        nsentences = logging_list[0]['nsentences']
        num = len(loss_list)
        for i in range(1, num):
            loss += loss_list[i].data
            loss_value += loss_list[i].data
            nll_loss += logging_list[i]['nll_loss'].data
            ntokens += logging_list[i]['ntokens']
            nsentences += logging_list[i]['nsentences']
        loss = loss / sample_size
        loss_value = loss_value / num
        nll_loss = nll_loss / sample_size
        ntokens = ntokens / num
        nsentences = nsentences / num
        return loss, loss_value, nll_loss, ntokens, nsentences
