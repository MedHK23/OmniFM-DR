...
class ImageClassifyDataset(OFADataset):
    def __init__(
        ...
        if self.split != 'train':
            ...
            self.patch_resize_transform = transforms.Compose(functools.reduce(lambda x, y:x + y, [
                ...
                [self.patch_resize_transform.transforms[2]],
                [RandomAugment(2, 2, isPIL=True, augs=['Identity', 'AutoContrast', 'Equalize', 'Brightness', 'ShearX', 'ShearY', 'TranslateX', 'TranslateY', 'Rotate']), ],
    ...
    def __getitem__(self, index):
        ...
        src_item = self.encode_text(' what kinds of disease does this chest DR image have?')
        tgt_item = self.encode_text(" {}".format(label_name))
        # print(f"Source item: {label_name, tgt_item}")
