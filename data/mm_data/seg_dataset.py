...
class SegDataset(OFADataset):
    def __init__(
        ...
        if type(bpe).__name__ == 'GPT2BPE':
            ...
            self.prompt = ' what region of the chest represents the contour of the "{} lung" ?'  # sunhao add 'lung'
            self.prompt = ' what region of the chest represents the contour of the "{}" ?'
    ...
    def __getitem__(self, index):
        ...
        region_list = [float(x) for x in nodes]
        region_list = [float(int(x*0.512)) for x in region_list]
        # if len(region_list) != 60:
        #     extend = [0] * (60 - len(region_list))
        #     region_list.extend(extend)

        def coord_augment(region_list):
            num_move=random.randint(0,len(region_list)//2-1)
            a=region_list[:2*num_move]
            # print('a:',a)
            b=region_list[2*num_move:]
            # print('b:',b)
            b.extend(a)
            # print('b+a:',b)
            return b

        def flip_augment(image,coord_list,text,p=0.5):
            if random.random()<p:
                ...
                for i in range(len(coord_list)):
                    if i%2==0:
                        coord_list[i]=w-coord_list[i]
            return image,coord_list,text
        
        # print('width:',w)
        # print('ori:',region_list)
        # #
        region_list=coord_augment(region_list)

        # print('roll aug:',region_list)

        # image,region_list,text=flip_augment(image,region_list,text)

        # print('flip aug:',region_list)
        # print('caption:',text)
        # exit()
