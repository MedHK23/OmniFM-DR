...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_embedding(cls, args, dictionary, embed_dim, path=None):
        # extra_dictionary = 9219  # TODO: should not be hard code (NO! Just delete)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if getattr(self.args, "cls_token", False):
    ...
    def get_patch_images_info(self, patch_images, sample_patch_num, device):
        ...
        if getattr(self.args, "interpolate_position", False) and image_num_patches > orig_num_patches:
            ...
            ## add classification token
            if getattr(self.args, "cls_token", False):
                image_position_ids = torch.cat([
                    torch.zeros(image_position_ids.shape[0], 1).to(device).to(image_position_ids.dtype),
                    image_position_ids
                ], dim=1)
                image_padding_mask = torch.cat([
                    torch.zeros(image_padding_mask.shape[0], 1).to(device).to(image_padding_mask.dtype),
                    image_padding_mask
                ], dim=1)
    ...
    def forward_embedding(
        ...
        if image_embed is not None:
            ...
            ## add classification token
            if getattr(self.args, "cls_token", False):
                cls_tokens = self.cls_token.expand(image_embed.shape[0], -1, -1).to(image_embed.device).to(image_embed.dtype)
    ...
    def forward_scriptable(
        ...
        # print(f"[--------------------]")
        # print(f" encoder_out: {x.shape}")
        # print(f" encoder_padding_mask : {encoder_padding_mask.shape}")
