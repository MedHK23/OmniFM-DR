...
from data.pretrain_data.joint_multi_instructs_dataset import DRJointDataset
...
class DRJointConfig(OFAConfig):
    ...
    report_negative_data: Optional[str] = field(
        default=None,
        metadata={"help": "report negative dataset"},
    )
    vqa_attritube_dataset_positive: Optional[str] = field(
        default=None,
        metadata={"help": "vqa disease attribute positive dataset"},
    )
    vqa_attritube_dataset_negative: Optional[str] = field(
        default=None,
        metadata={"help": "vqa disease attribute negative dataset"},
    )
    ...
    vg_selected_cols: Optional[str] = field(
        default=None,
        metadata={"help": "vg data selected cols"},
    )
    ...
    vqa_attritube_selected_cols: Optional[str] = field(
        default=None,
        metadata={"help": "disease attribute vqa selected cols"},
    )
...
class DRJointTask(OFATask):
    def __init__(self, cfg: DRJointConfig, src_dict, tgt_dict):
        ...
        if self.cfg.vg_data is not None:
            self.visual_ground_dataset = FileDataset(self.cfg.vg_data, self.cfg.vg_selected_cols)
        if self.cfg.vg_multi_data is not None:
            self.visual_ground_multiple_dataset = FileDataset(self.cfg.vg_multi_data, self.cfg.vg_selected_cols)
        ...
        if self.cfg.report_negative_data is not None:
            self.negative_dataset = FileDataset(self.cfg.report_negative_data, self.cfg.selected_cols)
        if self.cfg.vqa_attritube_dataset_positive is not None:
            self.vqa_attribute_positive = FileDataset(self.cfg.vqa_attritube_dataset_positive, self.cfg.vqa_attritube_selected_cols)
        if self.cfg.vqa_attritube_dataset_negative is not None:
            self.vqa_attribute_negative = FileDataset(self.cfg.vqa_attritube_dataset_negative, self.cfg.vqa_attritube_selected_cols)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = DRJointDataset(
            ...
            vqa_attritube_dataset_positive=self.vqa_attribute_positive,
            vqa_attritube_dataset_negative=self.vqa_attribute_negative,
