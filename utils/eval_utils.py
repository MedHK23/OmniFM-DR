...
def eval_vqa_gen(task, generator, models, sample, **kwargs):
    ...
    ######## sunhao 230628
    predicts = valid_result.tolist()
    hyps_new = []
    hyps = []
    for i in predicts:
        cur_ans = []
        posi = True
        for _cat, _score in enumerate(i):
            if _score > 0.5:
                cur_ans.append(task.index2ans[_cat]+'_'+'{:.4f}'.format(_score))
        if cur_ans == []:
            cur_ans.append('No Finding_100')
        cur_ans = ','.join(cur_ans)
        hyps_new.append(cur_ans)
        hyps.append(cur_ans.split('_')[0])
    results = [{"question_id": int(id), "answer": hyp} for id, hyp in zip(sample["id"].tolist(), hyps_new)]
    ########
...
def eval_seg(task, generator, models, sample, **kwargs):
    def _calculate_ap_score(hyps, refs, thresh=0.5):
        interacts = torch.cat(
            [torch.where(hyps[:, :2] < refs[:, :2], refs[:, :2], hyps[:, :2]),
             torch.where(hyps[:, 2:] < refs[:, 2:], hyps[:, 2:], refs[:, 2:])],
    ...
    # results = [
    #     {"uniq_id": sample_id,
    #      "contour": [hyps[i][0].item(), hyps[i][1].item(), hyps[i][2].item(), hyps[i][3].item()]}
    #     for i, sample_id in enumerate(sample["id"].tolist())
    # ]
    # scores = _calculate_ap_score(hyps, sample['region_coords'].float())
    scores = None
...
def eval_step(task, generator, models, sample, **kwargs):
    ...
    elif task.cfg._name == 'seg':
        return eval_seg(task, generator, models, sample, **kwargs)
    else:
